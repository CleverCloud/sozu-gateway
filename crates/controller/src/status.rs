...
mod tests {
    ...
    fn status_for_section<'a>(
        parents: &'a [RouteParentStatus],
        section: &str,
    ) -> &'a RouteParentStatus {
        parents
            .iter()
            .find(|parent| parent.parent_ref.section_name.as_deref() == Some(section))
            .unwrap()
    }
    ...
    fn l4_conflict_is_reported_only_on_the_affected_parent() {
        for (kind, protocol) in [(RouteKind::TcpRoute, "TCP"), (RouteKind::UdpRoute, "UDP")] {
            ...
            let accepted = &status_for_section(&parents, "shared").conditions[0];
            ...
            assert_eq!(
                status_for_section(&parents, "dedicated").conditions[0].message,
    ...
    fn l4_conflict_messages_are_stable_across_reconciles() {
        ...
        r.kind = RouteKind::TcpRoute;
        let mut first = route_parents(controller, &r, &[], Some(3), &GatewayScope::default());
    ...
    fn l4_conflict_message_clears_when_the_route_can_forward() {
        ...
        let cleared = route_parents(controller, &r, &first, Some(3), &GatewayScope::default());
        let first_shared = status_for_section(&first, "shared");
        let cleared_shared = status_for_section(&cleared, "shared");
        let accepted = &cleared_shared.conditions[0];
        assert_eq!(accepted.status, "True");
        assert_eq!(accepted.reason, "Accepted");
        assert_eq!(accepted.message, "Route accepted by sozu-gateway");
        assert_ne!(
            cleared_shared.conditions[0].message,
            first_shared.conditions[0].message
        );
        assert_eq!(cleared_shared.conditions[1], first_shared.conditions[1]);
        assert_eq!(
            status_for_section(&cleared, "dedicated"),
            status_for_section(&first, "dedicated")
        );
        assert_eq!(
            route_parents(controller, &r, &cleared, Some(3), &GatewayScope::default()),
            cleared
        );
    }
    ...
    fn gateway_listener_acceptance_conditions_are_correct_and_stable() {
        for (protocols, parameters, awaiting_address, accepted, reason) in [
            (vec!["HTTP"], false, false, "True", "Accepted"),
            (vec!["HTTP"], false, true, "True", "Accepted"),
            (
            ...
            (vec!["HTTP"], true, false, "False", "InvalidParameters"),
            (vec!["HTTP"], true, true, "False", "InvalidParameters"),
        ] {
            ...
            let mut conditions = build_gateway_conditions(gateway, &current, awaiting_address);
            ...
            if awaiting_address && gateway.programmed {
                assert_eq!(conditions[1].status, "False");
                assert_eq!(conditions[1].reason, "AddressNotAssigned");
            }
            if parameters {
                assert_eq!(conditions[1].status, "False");
                assert_eq!(conditions[1].reason, "Invalid");
                assert!(conditions[0]
                    .message
            ...
            let rebuilt = build_gateway_conditions(gateway, &current, awaiting_address);
            assert!(
            ...
            let updated = build_gateway_conditions(gateway, &current, awaiting_address);
