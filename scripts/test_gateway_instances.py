...
class GatewayProvisioning(unittest.TestCase):
    ...
    def render(self, release="sozu", values_files=(), **values):
        ...
        output = subprocess.run([
            ...
            *[arg for path in values_files for arg in ("--values", str(path))],
    ...
    def test_provisioned_workers_inherit_multiple_listeners(self):
        resources = self.render(gatewayProvisioning={"enabled": True},
            values_files=[CHART / "ci/multiple-listeners.yaml"])
        template = config(resources)
        ...
        for deployment, service, config_map in [
            ...
            with self.subTest(deployment=deployment["metadata"]["name"]):
                ...
                self.assertEqual([(e["name"], e["port"], e["bind"], e["protocol"].lower())
                                  for e in exposure], expected)
                self.assertEqual([(p["name"], p["port"], p["targetPort"]) for p in service["spec"]["ports"]],
                                 [(name, port, bind) for name, port, bind, _ in expected])
                sozu = next(c for c in deployment["spec"]["template"]["spec"]["containers"]
                            if c["name"] == "sozu")
                self.assertEqual([(p["name"], p["containerPort"]) for p in sozu["ports"]],
